...
def analyze_changepoints_to_file(
    standardized_changepoints_dict, save_path, stim_time=2000
):
        ...
        for dataset_name, taste_array_list in standardized_changepoints_dict.items():
            ...
            for taste_idx, trial_array in enumerate(taste_array_list):
                ...
                for i, cp in enumerate(cp_list):
                    ...
                    f.write(
                        f"    CP{i+1} stats (relative to stimulus at {stim_time} ms):\n"
                    )
                    f.write(f"      Mean:  {mean_cp:.2f} ms\n")
                    f.write(f"      Var:   {var_cp:.2f}\n")
                    f.write(f"      Std:   {std_cp:.2f}\n")
                    f.write(
                        f"      Most extreme high: {max_val} ms (deviation: {max_dev:+.2f})\n"
                    )
                    f.write(
                        f"      Most extreme low:  {min_val} ms (deviation: {min_dev:+.2f})\n"
                    )
