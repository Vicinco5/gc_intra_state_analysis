...
def plt_single_neuron_firing_unwarped_heatmap_thresholded(
    thresh_firing_rates_unwarped,
    dataset_num,
    output_dir_unwarped,
    window_length,
    step_size,
    ...
    num_neurons = thresh_firing_rates_unwarped[0][0].shape[
        1
    ]  # Assuming consistent neuron count across tastes and changepoints
    num_trials = thresh_firing_rates_unwarped[0].shape[
        0
    ...
    for neuron_idx in range(num_neurons):
        ...
        fig.suptitle(
            f"Unwarped Single Neuron Firing Rates, Threshold: 1 hz, Neuron: {neuron_idx + 1}, Dataset: {dataset_num}",
            fontsize=16,
            y=0.995,
        )
        # Ensure axes is 2D
        ...
        for taste_idx in range(num_tastes):
            ...
            for cp_idx in range(num_changepoints):
                ...
                for trial_idx in range(num_trials):  # Loop through trials
                    trial_array = thresh_firing_rates_unwarped[taste_idx][trial_idx][
                        cp_idx
                    ][neuron_idx].values
                ...
                if trial_data:
                    ...
                    im = ax.imshow(
                        ...
                        cmap="viridis",
                        origin="lower",
                        extent=[0, max_length * step_size, 0, num_non0_trials],
                    )
...
def plt_single_neuron_firing_unwarped_line_thresholded(
    sig_nrns_dict, output_dir, modified_tastes, epoch_labels, step_size
):
    ...
    for df_name, df in sig_nrns_dict.items():
        ...
        for dataset_num in unique_datasets:
            ...
            neuron_threshold_df = dataset_df.group_by("neuron").agg(
                pl.col(threshold_col_name).mean().alias(threshold_col_name)
            )
            neuron_threshold_dict = {
                row[0]: row[1] for row in neuron_threshold_df.rows()
            }

            neuron_threshold_values = np.array(list(neuron_threshold_dict.values()))
            neuron_percentiles = np.percentile(
                neuron_threshold_values, [25, 50, 75, 90]
            )
            ...
            for neuron_idx in unique_neurons:
                ...
                if neuron_value is not None:
                    fig.suptitle(
                        f"Warped Single Neuron Firing Rates, Threshold: 2Hz, Neuron: {neuron_idx + 1}, Dataset: {dataset_num}, {threshold_col_name}: {neuron_value:.2f}",
                        fontsize=14,
                        y=0.995,
                    )
                else:  # change between warped and unwarped as needed-- there should be a better way to do this but hey eh
                    fig.suptitle(
                        f"Warped Single Neuron Firing Rates, Threshold: 2Hz, Neuron: {neuron_idx + 1}, Dataset: {dataset_num}",
                        fontsize=14,
                        y=0.995,
                    )
                ...
                for taste_idx in range(num_tastes):
                    for cp_idx in range(num_changepoints):
                        ...
                        if trial_data:
                            ...
                            sorted_lengths = sorted(trial_lengths)
                            avg_duration = (
                                np.mean(sorted_lengths) * step_size
                            )  # Onset of stimulus
                ...
                neuron_fig_path = os.path.join(
                    dataset_dir, f"{percentile_score}_pct_neuron_{neuron_idx + 1}.png"
                )
                plt.savefig(neuron_fig_path)
                plt.close(fig)
    ...
    for df_name, df in sig_nrns_dict.items():
        ...
        for dataset_num in unique_datasets:
            ...
            for neuron_idx in unique_neurons:
                ...
                fig.suptitle(
                    f"Mean Firing Rates with SEM, Neuron: {neuron_idx + 1}, Dataset: {dataset_num}",
                    fontsize=14,
                    y=0.995,
                )
                ...
                plt.tight_layout()
                neuron_fig_path = os.path.join(
                    dataset_dir, f"mean_with_sem_neuron_{neuron_idx + 1}.png"
                )
                plt.savefig(neuron_fig_path)
                plt.close(fig)
...
def plot_neuron_mean_with_sem_grid(
    sig_nrns_dict, output_dir, modified_tastes, epoch_labels, step_size
):
    num_tastes = len(modified_tastes)
    num_changepoints = 4  # Number of changepoints; hardcoded but can be made flexible
    ...
    for df_name, df in sig_nrns_dict.items():
        ...
        for dataset_num in unique_datasets:
            ...
            for taste_idx in range(num_tastes):
                for cp_idx in range(num_changepoints):
                    fig_num = 1
                    for neuron_start in range(
                        0, len(unique_neurons), neurons_per_figure
                    ):
                        fig, axes = plt.subplots(
                            4, 4, figsize=(20, 20), sharex=False, sharey=False
                        )
                        fig.suptitle(
                            f"Mean Unwarped Firing Rates with SEM, Epoch: {epoch_labels[cp_idx]}, Taste: {taste_labels[taste_idx]}, Dataset: {dataset_num}",
                            fontsize=18,
                            y=0.995,
                        )
                        ...
                        for i, neuron_idx in enumerate(
                            unique_neurons[
                                neuron_start : neuron_start + neurons_per_figure
                            ]
                        ):
                            row = i // 4
                            col = i % 4
                            ...
                            for trial_idx in unique_trials:
                                trial_row = filtered_df.filter(
                                    pl.col("trial") == trial_idx
                                )
                                if not trial_row.is_empty():
                                    trial_array = np.array(trial_row["trial_data"][0])
                                    valid_data = trial_array[~np.isnan(trial_array)]
                            ...
                            if trial_data:
                                ...
                                # Calculate the mean and standard error of the mean (SEM)
                                all_data = np.full(
                                    (len(trial_data), max_time_bins), np.nan
                                )
                        ...
                        for i, neuron_idx in enumerate(
                            ...
                            try:
                                ...
                                if valid_trials:
                                    ...
                                    all_data = np.full(
                                        (len(valid_trials), max_bins), np.nan
                                    )
...
def plot_population_activity(
    ...
    num_tastes = len(modified_tastes)
    num_changepoints = (
        4  # Updated number of changepoints to include poststimulus activity
    )
    ...
    for df_name, df in sig_nrns_dict.items():
        ...
        for dataset_num in unique_datasets:
            ...
            fig.suptitle(
                f"Population Activity, Dataset: {dataset_num}", fontsize=14, y=0.995
            )
            ...
            for taste_idx in range(num_tastes):
                for cp_idx in range(num_changepoints):
                    ...
                    ax.set_title(
                        f"Taste: {modified_tastes[taste_idx]}, Epoch: {epoch_labels[cp_idx]}"
                    )
                    ax.set_xlabel("Time Duration of State")
                    if cp_idx == 0:  # Only add Y label to the first column
                        ax.set_ylabel("Firing Rate (Hz)")
                    ...
                    if all_neuron_data:
                        ...
                        all_data = np.full(
                            (len(all_neuron_data), max_time_bins), np.nan
                        )
                        for i, data in enumerate(all_neuron_data):
                            all_data[i, : len(data)] = data

                        mean_neuron_data = np.nanmean(all_data, axis=0)
                        sem_neuron_data = np.nanstd(all_data, axis=0) / np.sqrt(
                            np.sum(~np.isnan(all_data), axis=0)
                        )
                        mean_time_bins = np.arange(max_time_bins) * step_size
                        ...
                        ax.fill_between(
                            mean_time_bins,
                            mean_neuron_data - sem_neuron_data,
                            mean_neuron_data + sem_neuron_data,
                            color="gray",
                            alpha=0.5,
                            label="SEM",
