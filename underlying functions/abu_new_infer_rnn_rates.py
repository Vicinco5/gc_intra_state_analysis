...
# Load JSON config
config_path = (
    "/home/vincent/Senior thesis work/blechRNN-master/config/blechrnn_config.json"
)
...
parser.add_argument(
    "--train_steps", type=int, default=15000, help="Number of training steps"
)
parser.add_argument("--hidden_size", type=int, default=8, help="Hidden size of RNN")
parser.add_argument(
    "--bin_size", type=int, default=25, help="Bin size for binning spikes"
)
parser.add_argument(
    "--train_test_split", type=float, default=0.75, help="Fraction of data for training"
)
parser.add_argument("--no_pca", action="store_true", help="Skip PCA preprocessing")
...
parser.add_argument(
    ...
    nargs=2,
    default=[1500, 4500],
    help="Time limits for inferred firing rates",
)
# check when we start training
...
import visualize as vz  # Adjust if it's a function within ephys_data
...
for file in os.listdir(h5_dir):
    if file.endswith(".h5"):
        h5_filepath = os.path.join(h5_dir, file)
        dataset_name = os.path.splitext(file)[0]

        print(f"Processing data from {h5_filepath}")

        # Set output paths specific to each file
        ...
        # Load data using ephys_data
        data = ephys_data(
            os.path.dirname(h5_filepath)
        )  # Updated to process specific file
        data.get_spikes()

        # Add the BlechRNN paths for model imports
        ...
        for taste_ind, taste_spikes in enumerate(spike_array):
            ...
            binned_spikes = np.reshape(
                taste_spikes, (*taste_spikes.shape[:2], -1, bin_size)
            ).sum(-1)
            ...
            inputs_plus_context = np.concatenate(
                [inputs, stim_time[:, :, None], trial_num_broad[:, :, None]], axis=-1
            )
            ...
            fig.savefig(
                os.path.join(plots_dir, f"run_loss_taste_{taste_ind}.png"),
                bbox_inches="tight",
            )
            plt.close(fig)
            ...
            # Latent factors
            fig, ax = plt.subplots(
                latent_outs.shape[-1], 1, figsize=(5, 10), sharex=True, sharey=True
            )
            for i in range(latent_outs.shape[-1]):
                ax[i].imshow(latent_outs[..., i].T, aspect="auto")
            plt.suptitle("Latent Factors")
            fig.savefig(
                os.path.join(plots_dir, f"latent_factors_taste_{taste_ind}.png")
            )
            plt.close(fig)
            ...
            fig, ax = plt.subplots(1, 2)
            ax[0].imshow(
                zscore(pred_firing_mean, axis=-1), aspect="auto", interpolation="none"
            )
            ax[1].imshow(
                zscore(binned_spikes_mean, axis=-1), aspect="auto", interpolation="none"
            )
            ax[0].set_title("Pred")
            ax[1].set_title("True")
            fig.savefig(
                os.path.join(plots_dir, f"mean_firing_zscored_taste_{taste_ind}.png")
            )
            plt.close(fig)
            ...
            for i in range(binned_spikes.shape[1]):
                fig, ax = plt.subplots(
                    3, 1, figsize=(10, 10), sharex=True, sharey=False
                )
            ...
            for i in range(latent_outs.shape[1]):
                ...
                fig.savefig(
                    os.path.join(
                        trial_latent_dir, f"taste_{taste_ind}_trial_{i}_latent.png"
                    )
                )
                plt.close(fig)
        ...
        for i in range(binned_spikes.shape[1]):
            ...
            this_cat_spikes = np.concatenate(this_spikes_list)[
                ..., time_lims[0] : time_lims[1]
            ]
            ...
            for j in range(mean_conv_rate.shape[0]):
                ax[1].plot(conv_x, mean_conv_rate[j].T, c=cmap(j), linewidth=2)
                ax[1].fill_between(
                    conv_x,
                ...
                # ax[2].plot(binned_x, binned_spikes[:,i].T, label = 'True')
                ax[2].plot(binned_x[1:], mean_pred_firing[j].T, c=cmap(j), linewidth=2)
                ax[2].fill_between(
                    binned_x[1:],
            ...
            ax[2].set_title("RNN Predicted Firing Rate")
            fig.savefig(
                os.path.join(ind_plot_dir, f"neuron_{i}_mean_raster_conv_pred.png")
            )
            plt.close(fig)
        ...
        with tables.open_file(hdf5_path, "r+") as hf5:
            # Create directory for rnn output if not present
            if "/rnn_output" not in hf5:
                hf5.create_group("/", "rnn_output", "RNN Output")
            rnn_output = hf5.get_node("/rnn_output")

            # Write out latents and predicted firing rates for each taste in separate arrays
            for taste_ind, (pred_firing, latent_out) in enumerate(
                zip(pred_firing_list, latent_out_list)
            ):
                taste_group_name = f"taste_{taste_ind}"

                # Check if group already exists and remove if so
                if hasattr(rnn_output, taste_group_name):
                    hf5.remove_node(rnn_output, taste_group_name, recursive=True)

                # Create a new group for the taste
                taste_grp = hf5.create_group(
                    rnn_output, taste_group_name, f"Taste {taste_ind}"
                )

                # Save arrays in the taste group
        ...
        print("Saved RNN outputs to HDF5 file.")

        # saving latents to parquet files
        # Prepare a list to collect data for each taste
        all_latents = []

        # Process each taste's latent output and prepare it for saving
        for taste_index, latent_out in enumerate(latent_out_list):
            # `latent_out` is shaped (trial, time, latent_dim), no summarization over time
            num_trials, num_time_steps, latent_dim = latent_out.shape

            # Reshape and convert to DataFrame
            reshaped_latents = latent_out.reshape(
                -1, latent_dim
            )  # Shape: (trial * time, latent_dim)

            # Create DataFrame and add latent dimensions
            latent_df = pl.DataFrame(
                reshaped_latents, schema=[f"latent_dim_{i}" for i in range(latent_dim)]
            )

            # Generate columns for taste, trial, and time indices
            taste_column = pl.Series("taste", [taste_index] * len(latent_df))
            trial_column = pl.Series(
                "trial", np.repeat(np.arange(num_trials), num_time_steps)
            )
            time_column = pl.Series(
                "time", np.tile(np.arange(num_time_steps), num_trials)
            )

            # Add these columns to DataFrame
            latent_df = latent_df.with_columns(
                [taste_column, trial_column, time_column]
            )

            # Append to the list
            all_latents.append(latent_df)

        # Concatenate all tastes' DataFrames into a single DataFrame
        combined_latents_df = pl.concat(all_latents)

        # Define output path for the Parquet file
        latent_output_path = os.path.join(
            artifacts_dir, f"{dataset_name}_raw_latent_vectors.parquet"
        )

        # Save to Parquet file
        combined_latents_df.write_parquet(latent_output_path)

        print(f"Saved raw latent vectors to {latent_output_path}")
