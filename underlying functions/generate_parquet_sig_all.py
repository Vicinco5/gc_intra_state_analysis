...
from interpolation_xr import (
    interpolate_and_average_all_tastes,
    interpolate_firing_rates,
)


def sig_neurons_ttest(
            ...
            if extracted_pkl is not None:
                ...
                try:
                    ...
                    (
                        ...
                        spike_array, changepoints, window_length, step_size
                    )
                    ...
                    significant_neurons = (
                        set()
                    )  # Track neurons found significant in any epoch
                    pvals_for_neurons = (
                        {}
                    )  # Dictionary to track p-values for significant neurons
                        ...
                        for cp_idx in range(num_changepoints):
                            for neuron_idx in range(num_neurons):
                                ...
                                for trial_idx in range(num_trials):
                                    trial_array = state_firing_rates_all_trials[
                                        taste_idx, trial_idx, cp_idx, neuron_idx, :
                                    ].values
                                    valid_data = trial_array[~np.isnan(trial_array)]
                                    if valid_data.size > 0:
                                        half_point = len(valid_data) // 2
                                        mean_first_half = np.mean(
                                            valid_data[:half_point]
                                        )
                                        mean_second_half = np.mean(
                                            valid_data[half_point:]
                                        )
                    ...
                    for neuron_idx in significant_neurons:
                        for taste_idx in range(num_tastes):
                            for cp_idx in range(num_changepoints):
                                for trial_idx in range(num_trials):
                                    trial_data = state_firing_rates_all_trials[
                                        taste_idx, trial_idx, cp_idx, neuron_idx, :
                                    ].values
                                    trial_data_cleaned = trial_data[
                                        ~np.isnan(trial_data)
                                    ]  # Remove NaN values
                    ...
                    if (
                        len(all_sig_neuron_dataframes) > 10000
                    ):  # Arbitrary large chunk size to prevent memory issues
                        temp_df = pl.DataFrame(all_sig_neuron_dataframes)
                        if "consolidated_ttest_df" in locals():
                            consolidated_ttest_df = pl.concat(
                                [consolidated_ttest_df, temp_df], rechunk=True
                            )
                        else:
        ...
        if isinstance(data, tuple):
            ...
            if extracted_pkl is not None:
                ...
                try:
                    changepoints = extracted_pkl[:, 3]
                    (
                        state_firing_rates_uw_all_trials,
                        state_spike_arrays_all_trial,
                        state_firing_rates_warped,
                        state_spike_arrays_warped,
                    ) = calc_fr_states(
                        spike_array, changepoints, window_length, step_size
                ...
                try:
                    ...
                    if (
                        ...
                        if "consolidated_hz_df" in locals():
                            consolidated_hz_df = pl.concat(
                                [consolidated_hz_df, temp_df], rechunk=True
                            )
                        else:
...
def sig_neurons_war_hz(
    npz_path, pkl_path, sig_hz_parquet_path_w, threshold_hz, window_length, step_size
):
...
def consolidate_all_neuron_data(
...
def consolidate_all_neuron_war_data(
