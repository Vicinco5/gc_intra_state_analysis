...
# Bin spikes
bin_size = 25
binned_spikes = np.reshape(cat_spikes, (*cat_spikes.shape[:2], -1, bin_size)).sum(-1)
...
inputs_plus_context = np.concatenate([inputs, stim_time[:, :, None]], axis=-1)
...
train_inds = np.random.choice(
    np.arange(inputs.shape[1]), int(train_test_split * inputs.shape[1]), replace=False
)
...
fig.savefig(os.path.join(plot_dir, "run_loss.png"), bbox_inches="tight")
plt.close(fig)

# Plot another figure of cross_val_loss
fig, ax = plt.subplots()
for ind, loss in enumerate(cross_val_loss_list):
    ax.plot(loss.keys(), loss.values(), label=model_name)
ax.legend(bbox_to_anchor=(1.05, 1), loc="upper left", borderaxespad=0.0)
ax.set_title(f"Cross Val Losses")
fig.savefig(os.path.join(plot_dir, "cross_val_loss.png"), bbox_inches="tight")
plt.close(fig)
