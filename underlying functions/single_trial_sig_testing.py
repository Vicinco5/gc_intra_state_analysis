...
def thresh_min_fr_unwarped_hz(state_firing_rates_all_trials, threshold_hz):
    ...
    thresh_hz_state_firing_rates = xr.DataArray(
        ...
        coords={
            ...
            "time_bin": range(
                state_firing_rates_all_trials[0][0].shape[-1]
            ),  # maximum possible bins
        },
    )
        ...
        for taste_idx in range(num_tastes):
            for cp_idx in range(num_changepoints):
                for trial_idx in range(num_trials):  # Loop through trials
                    ...
                    if (
                        valid_data.size > 0
                    ):  # Only process if there are valid (non-NaN) values
                        avg_fr = np.nanmean(valid_data)
                        if (
                            avg_fr >= threshold_hz
                        ):  # threshold firing rate of 1 hz over the entire recording
                            thresh_hz_state_firing_rates[
                                taste_idx, trial_idx, cp_idx, neuron_idx, :
                            ] = state_firing_rates_all_trials[taste_idx][trial_idx][
                                cp_idx
                            ][
                                neuron_idx
                            ]
            ...
            for cp_idx in range(num_changepoints):
                for trial_idx in range(num_trials):  # Loop through trials
                    ...
                    if (
                        ...
                        ttest_b = valid_data[
                            num_tbin_ttest : num_tbin_ttest + len(ttest_a)
                        ]
                        if len(ttest_a) == len(ttest_b):
                            tstat, pval = ttest_rel(ttest_a, ttest_b)
                            if pval < alpha and not np.isnan(
                                pval
                            ):  # thresholding for significance of firing via t-test
                                thresh_ttest_state_firing_rates[
                                    taste_idx, trial_idx, cp_idx, neuron_idx, :
                                ] = state_firing_rates_all_trials[taste_idx][trial_idx][
                                    cp_idx
                                ][
