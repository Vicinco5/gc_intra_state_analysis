    ...
    def run(self):
        ...
        for taste_idx in range(taste_num):
            ...
            for trial_idx in range(trial_num):
                ...
                if (
                    not isinstance(trial_changepoints, np.ndarray)
                    or np.isnan(trial_changepoints).any()
                ):
                    print(
                ...
                start_time = (
                    1500  # 2000 is stim delivery--- change to 1500 for time prior to
                )
                for cp_idx, changepoint in enumerate(trial_changepoints):
                    ...
                    if end_time <= start_time:
                        print(
                            f"No data for segment from {start_time} to {end_time}. Skipping..."
                        )
                        continue
                    ...
                    if self.compute_unwarped_firing_rates:
                        if segment_unwarped.shape[1] > 0:  # non-empty
                            unwarped_fr = self.calc_fr_rr(
                                segment_unwarped, window_length, step_size
                            )
                        else:
                            unwarped_fr = None
                    else:
                    ...
                    if (
                        self.compute_warped_spike_arrays
                        or self.compute_warped_firing_rates
                    ):
                        if segment_unwarped.shape[1] < 2:
                            ...
                            war_fr = None
                            print(
                                f"Not enough points to interpolate for segment {cp_idx}. Skipping warp..."
                            )
                        else:
                            ...
                            war_segment = np.zeros(
                                (segment_unwarped.shape[0], len(war_len))
                            )
                            ...
                            if self.compute_warped_firing_rates:
                                war_fr = self.calc_fr_rr(
                                    war_segment, window_length, step_size
                                )
                            else:
                                war_fr = None
                    else:
                    ...
                    if (
                        self.compute_unwarped_spike_arrays
                        and segment_unwarped.shape[1] > 0
                    ):
                        # segment_unwarped.shape[1] == end_time - start_time
                        self.state_spike_arrays.loc[
                    ...
                    if self.compute_unwarped_firing_rates and (unwarped_fr is not None):
                        ...
                        if pad_width > 0:
                            ...
                            padded_fr = unwarped_fr[
                                :, :max_bins
                            ]  # truncate if it's too long

                        self.state_firing_rates.loc[
                            taste_idx, trial_idx, cp_idx, :, :
                        ] = padded_fr
                    ...
                    if self.compute_warped_spike_arrays and (war_segment is not None):
                        ...
                        self.state_spike_arrays_warped.loc[
                            taste_idx, trial_idx, cp_idx, :, : war_segment.shape[1] - 1
                        ] = war_segment
                    ...
                    if self.compute_warped_firing_rates and (war_fr is not None):
                        ...
                        self.state_firing_rates_warped.loc[
                            taste_idx, trial_idx, cp_idx, :, :
                        ] = padded_war_fr
