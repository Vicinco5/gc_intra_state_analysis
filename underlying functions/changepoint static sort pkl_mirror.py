...
from unpkl_generator import unpickle_changepoints, extract_valid_changepoints
...
from RNN_Intrastate_war_unwar_plotting_class import (
    RNNWarpingIntraAroundCP,
)  # own file now
...
results_summary = (
    tester.run()
)  # this kinda sucks and to find out why, we'll have to get into that later on, chuck


### NOW FOR THE FFT-- studying the oscillatory stuff
## all the above is hooey-- this is where we're at now:

from freuqency_analysis_suite_rnn_class import (
    FrequencyAnalysisPipeline,
)  # again now its own class
