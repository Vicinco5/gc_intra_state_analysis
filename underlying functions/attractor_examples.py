...
for i in range(X.shape[0]):
    for j in range(X.shape[1]):
        dists = [
            np.linalg.norm(np.array([X[i, j], Y[i, j]]) - attractor)
...
potential = lambda x, y: np.min(
    [np.linalg.norm(np.array([x, y]) - attractor) for attractor in attractors]
)
...
def update_state(state, attractor, omega):
    ...
    new_state = (
        attractor
        + np.array([r * np.cos(theta + omega), r * np.sin(theta + omega)])
        + noise
    )
    return new_state
