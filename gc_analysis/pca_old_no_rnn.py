...
def whitened_pca_all_nrns_sep_taste(all_data_df,pca_output_dir):
        ...
        for taste in unique_tastes:
            ...
            for changepoint in unique_changepoints:
                ...
                for neuron in unique_neurons:
                    neuron_df = taste_df.filter((pl.col('changepoint') == changepoint) & (pl.col('neuron') == neuron))
                ...
                if np.any(np.std(standardized_data, axis=0) > 0):
                    ...
                    all_pca_results.append((dataset, taste, changepoint, pca_result, explained_variance, singular_values, trial_lengths))
                else:
                    print(f"Skipping PCA for dataset {dataset}, taste {taste}, changepoint {changepoint} due to lack of variation.")
    ...
    pca_results_df = pl.DataFrame({
        ...
        'explained_variance_ratio': [result[4].tolist() for result in all_pca_results],
        'singular_values': [result[5].tolist() for result in all_pca_results],
        'trial_lengths': [result[6] for result in all_pca_results]  # Store trial lengths
    })
...
def plot_pca_components_by_trial(pca_results_df, pca_output_dir):
    ...
    for dataset in unique_datasets:
        ...
        for taste in unique_tastes:
            ...
            for changepoint in unique_changepoints:
                split_pca_results = unconcatenate_pca_results_taste(pca_results_df, dataset, taste, changepoint)
            ...
            for idx, trial_data in enumerate(final_trial_data):
                if trial_data.shape[0] < max_trial_length:
                    ...
                    padding = np.full((max_trial_length - trial_data.shape[0], trial_data.shape[1]), np.nan)
                    final_trial_data[idx] = np.concatenate((trial_data, padding), axis=0)
            ...
            for trial_idx, trial_data in enumerate(final_trial_data):
                ...
                plt.title(f'PCA Components for {dataset}, Taste: {taste}, Trial {trial_idx + 1}')
                plt.xlabel('Time Bins')
                plt.ylabel('PCA Value')
                plt.legend(loc='upper right', fontsize='small')
                
                # Save the figure
                plt.savefig(os.path.join(dataset_output_dir, f'{taste}_trial_{trial_idx + 1}_pca.png'))
                plt.close()
...
def plot_pca_components_averaged_over_tastes(pca_results_df, pca_output_dir):
    ...
    def interpolate_to_length(data, target_length):
        ...
        for i in range(data.shape[1]):  # Interpolate each principal component separately
            f = interp1d(x_original, data[:, i], kind='linear', fill_value='extrapolate')
    ...
    for dataset in unique_datasets:
        ...
        for changepoint in unique_changepoints:
            ...
            num_trials = len(taste_pca_data[0])  # Assuming all tastes have the same number of trials
            max_trial_length = max(max(len(trial) for trial in trials) for trials in taste_pca_data)
            averaged_trials = []
            
            for trial_idx in range(num_trials):
                # Interpolate each trial for all tastes to the maximum length
                interpolated_tastes = [interpolate_to_length(taste_pca_data[t][trial_idx], max_trial_length) for t in range(len(taste_pca_data))]
        ...
        final_trial_data = []
        max_trial_length = 0
        for trial_idx in range(len(dataset_trials[0])):  # Assuming consistent number of trials across changepoints
            concatenated_data = np.concatenate([dataset_trials[cp_idx][trial_idx] for cp_idx in range(len(unique_changepoints))], axis=0)
            max_trial_length = max(max_trial_length, concatenated_data.shape[0])
            final_trial_data.append(concatenated_data)
        ...
        for trial_idx, trial_data in enumerate(final_trial_data):
            ...
            plt.title(f'PCA Components Averaged Over Tastes for {dataset}, Trial {trial_idx + 1}')
            plt.xlabel('Time Bins')
            plt.ylabel('PCA Value')
            plt.legend(loc='upper right', fontsize='small')
            
            # Save the figure
            plt.savefig(os.path.join(dataset_output_dir, f'trial_{trial_idx + 1}_pca.png'))
            plt.close()
        ...
        for changepoint in unique_changepoints:
            ...
            for trial_idx in range(num_trials):
                ...
                for t in range(len(taste_pca_data)):
                    if len(taste_pca_data[t][trial_idx]) > 0:  # Ensure the trial is non-empty
                        interpolated_tastes.append(interpolate_to_length(taste_pca_data[t][trial_idx], max_trial_length))
...
def plot_pca_components_with_heatmap(pca_results_df, pca_output_dir):
    ...
    for dataset in unique_datasets:
        ...
        for trial_idx, trial_data in enumerate(final_trial_data):
            ...
            # Heatmap plot
            plt.figure(figsize=(10, 5))
            plt.imshow(trial_data[:, :8].T, aspect='auto', cmap='coolwarm', interpolation='none')
            plt.colorbar(label='PCA Value')
            plt.title(f'PCA Heatmap Averaged Over Tastes for {dataset}, Trial {trial_idx + 1}')
            plt.xlabel('Time Bins')
            plt.ylabel('Principal Components')
            plt.yticks(ticks=np.arange(8), labels=[f'PC {i + 1}' for i in range(8)])

            # Save the heatmap
            plt.savefig(os.path.join(dataset_output_dir, f'trial_{trial_idx + 1}_pca_heatmap.png'))
            plt.close()
