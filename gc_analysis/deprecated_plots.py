...
def plot_firing_rates(all_interpolated_states, firing_rate_arrays, taste_idx=0):
    ...
    for neuron_idx in range(num_neurons):
        for trial_idx in range(num_trials):
                  ...
                  fig, axs = plt.subplots(2, 3, figsize=(15, 10), constrained_layout=True)  # 2 rows, 3 columns per set
        ...
        for state_idx in range(3):  # Assuming 3 states as per the setup
                    ...
                    interpolated_data = np.array([trial[neuron_idx] for trial in all_interpolated_states[taste_idx][state_idx]])
                    median_length = interpolated_data.shape[2]  # Assuming time is the third dimension
                    ...
                    for interp_trial in interpolated_data:
                        axs[0, state_idx].plot(time_interpolated, interp_trial[trial_idx], alpha=0.5)
                    ...
                    actual_data = firing_rate_arrays[taste_idx][trial_idx][state_idx][neuron_idx, :]
...
def plot_firing_rates(all_interpolated_states, state_firing_rates, taste_idx=0):
    ...
    num_segments = interpolated_taste_data.sizes['segment']  # Assuming 3 states as per the setup

    # Iterate over each neuron and trial
    for neuron_idx in range(num_neurons):
        for trial_idx in range(num_trials):
            # Create a new figure for each neuron and trial with 2 rows, num_segments columns
            fig, axs = plt.subplots(2, num_segments, figsize=(15, 10), constrained_layout=True)
            fig.suptitle(f'Taste {taste_idx}, Neuron {neuron_idx}, Trial {trial_idx}')
            
            for state_idx in range(num_segments):
                # Get interpolated data for this state
                interpolated_data = interpolated_taste_data.sel(segment=state_idx, neuron=neuron_idx, trial=trial_idx)
                ...
                actual_data = actual_taste_data.sel(segment=state_idx, neuron=neuron_idx, trial=trial_idx)
...
# this works lol 
def plot_changepoints_spiketrain(dataset_num, state_spike_arrays_all_trials, window_length, step_size):
    ...
    for fig_idx in range(num_figures):
        ...
        fig.suptitle(f"Dataset {dataset_num} - Trials {fig_idx*5+1}-{min((fig_idx+1)*5, num_trials)}")
    ...
    for fig_idx in range(num_figures):
        ...
        for i in range(5):
            ...
            for cp_idx, state_spike_array in enumerate(trial_spike_arrays):
                ...
                tick_labels = [f"{start_time + t * step_size:.0f}" for t in range(num_ticks)]
                ax.set_xticks(range(num_ticks))  # Set ticks
                ax.set_xticklabels(tick_labels)  # Set tick labels
                ax.set_xlabel("Time")

                prev_end_time += state_spike_array.shape[1] + 1  # Update previous end time with padding
