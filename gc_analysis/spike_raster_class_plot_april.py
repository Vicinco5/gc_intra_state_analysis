...
"""
    ...
    def cli_options():
        ...
        mode = input("Select plotting mode ('cell-wise', 'trial-wise', or 'debug'): ").strip().lower()
        while mode not in ['cell-wise', 'trial-wise', 'debug']:
            mode = input("Invalid input. Choose 'cell-wise', 'trial-wise', or 'debug': ").strip().lower()
        ...
        if not is_warped:
            ...
            show_markers_input = input("Show start/end markers? (y/n): ").strip().lower()
            show_markers = show_markers_input == 'y'
    
            sort_input = input("Sort unwarped data by duration (longest first)? (y/n): ").strip().lower()
        ...
        if "segment" in self.data.dims:
            if "unified_time" not in self.data.dims:
                try:
                    ...
                    self.data = self.data.stack(unified_time=("segment", "original_time"))
    ...
    def _plot_wholedata_cellwise(self):
        ...
        if "taste" not in self.data.coords or "trial" not in self.data.coords or "neuron" not in self.data.coords:
        ...
        for taste_idx in self.data.coords["taste"].values:
            ...
            for ax_idx, neuron_idx in enumerate(neurons[:4]):  # Only plot 4 neurons per figure for now
                ax = axes[ax_idx]
                print(f"    Plotting Neuron {neuron_idx} on axis {ax_idx}")
    
                for trial_idx in trials:
                    try:
                        spikes = self.data.sel(taste=taste_idx, neuron=neuron_idx, trial=trial_idx).values
                    except Exception as e:
                        print(f"    Skipping trial {trial_idx} due to selection error: {e}")
                        continue
    
                    spike_times = self._extract_spike_times(spikes)
                    if len(spike_times) > 0:
                        ax.scatter(spike_times, [trial_idx] * len(spike_times), color='k', marker='|', s=20)
            ...
            fig.suptitle(f"Whole-Data Cell-Wise Raster\nTaste {taste_idx}, Dataset {self.dataset_name}", fontsize=14)
            fig.tight_layout(rect=[0, 0.05, 1, 0.95])
    
            # Legend (once per fig)
            red_line = mlines.Line2D([], [], color='red', linestyle='--', label='Stimulus')
            blue_line = mlines.Line2D([], [], color='blue', linestyle='--', label='Changepoint')
    ...
    def _plot_wholedata_trialwise(self):
        ...
        for taste_idx in self.data.coords["taste"].values:
            ...
            for chunk_start in range(0, len(trials), 4):
                ...
                for ax_idx, trial_idx in enumerate(trials[chunk_start:chunk_start + 4]):
                    ax = fig_axes[ax_idx]
                    max_time = 0
                    ...
                    for neuron_idx in neurons:
                        ...
                        for segment_idx in self.data.coords["segment"].values:
                            segment = self.data.sel(
                                taste=taste_idx, trial=trial_idx, neuron=neuron_idx, segment=segment_idx
                            ).values
                        ...
                        if spike_times.size > 0:
                            ax.scatter(spike_times, [neuron_idx] * len(spike_times), color='k', marker='|', s=20)
                    ...
                    ax.set_xticklabels([str(int(t)) for t in np.linspace(t_start, max_time, num=9)])
                    ax.set_title(f"Trial {trial_idx + 1}")
                    ax.set_xlabel("Time (ms)")
                    ax.set_ylabel("Neuron")
                    ax.tick_params(labelbottom=True)
    
                fig.suptitle(f"Whole-Data Trial-Wise Raster\nTaste {taste_idx}, Dataset {self.dataset_name}", fontsize=14)
                fig.tight_layout(rect=[0, 0.05, 1, 0.95])
            ...
            for epoch_idx in self.data.coords["segment"].values:
                ...
                for i, neuron_idx in enumerate(neuron_list):
                    ...
                    for trial_idx in trial_list:
                        spikes = self.data.sel(taste=taste_idx, trial=trial_idx, segment=epoch_idx, neuron=neuron_idx).values
                    ...
                    for j, (trial_idx, spikes) in enumerate(zip(trial_list, spike_rows)):
                        spike_times = np.where(spikes == 1)[0]
                        ax.scatter(spike_times, [j] * len(spike_times), color='k', marker='|', s=20)

                        if self.show_markers and self.changepoints_dict:
                            cp = self.changepoints_dict[self.dataset_name][taste_idx][trial_idx]
                            ax.scatter([cp[epoch_idx]], [j], color='blue', marker='|', s=20)
                            ax.scatter([cp[epoch_idx + 1]], [j], color='red', marker='|', s=20)

                    ax.set_title(f"Neuron {neuron_idx+1}")

                fig.suptitle(f"{self.mode.title()} Raster\nDataset {self.dataset_name}, Taste {taste_idx}, Epoch {epoch_idx}, {self.base_name}", fontsize=14)
                fig.tight_layout(rect=[0, 0.05, 1, 0.95])
                ...
                if self.show_markers:
                    ...
                    fig.legend(handles=[blue_marker, red_marker], loc='lower center', bbox_to_anchor=(0.5, 0))

                fig_filename = f"Taste_{taste_idx}_Epoch_{epoch_idx}_{self.dataset_name}.png"
        ...
        for taste_idx in self.data.coords["taste"].values:
            ...
            for epoch_idx in self.data.coords["segment"].values:
                ...
                for i, trial_idx in enumerate(trial_list):
                    ...
                    for j, (neuron_idx, spikes) in enumerate(zip(neuron_list, spike_rows)):
                        spike_times = np.where(spikes == 1)[0]
                        ax.scatter(spike_times, [j] * len(spike_times), color='k', marker='|', s=20)
    ...
    @staticmethod
    def extract_sample_spike_array_from_calc(calc_obj, taste=0, trial=0, neuron=0, segment=None):
        """
        ...
        if calc_obj.spike_arrays_unwarped is not None:
            return SpikeRasterPlotter.extract_sample_spike_array(calc_obj.spike_arrays_unwarped, taste, trial, neuron, segment)
        elif calc_obj.spike_arrays_warped is not None:
            return SpikeRasterPlotter.extract_sample_spike_array(calc_obj.spike_arrays_warped, taste, trial, neuron, segment)
        else:
            raise ValueError("No spike arrays available in the CalcFRStates object.")
    
    @staticmethod
    def extract_sample_spike_array(data_array: xr.DataArray, taste=0, trial=0, neuron=0, segment=None):
        """
        ...
        if segment is not None and "segment" in data_array.dims:
            return data_array.sel(taste=taste, trial=trial, neuron=neuron, segment=segment).values
        else:
