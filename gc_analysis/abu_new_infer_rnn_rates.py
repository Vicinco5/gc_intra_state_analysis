...
parser.add_argument('--override_config', action='store_true', help='Override config file and use provided arguments')
...
parser.add_argument('--bin_size', type=int, default=25, help='Bin size for binning spikes')
parser.add_argument('--train_test_split', type=float, default=0.75, help='Fraction of data for training')
...
parser.add_argument('--time_lims', type=int, nargs=2, default=[1500, 4500], help='Time limits for inferred firing rates')
...
for file in os.listdir(h5_dir):
    if file.endswith(".h5"):
        ...
        data = ephys_data(os.path.dirname(h5_filepath))  # Updated to process specific file
        data.get_spikes()
        ...
        with tables.open_file(hdf5_path, 'r+') as hf5:
            ...
            for taste_ind, (pred_firing, latent_out) in enumerate(zip(pred_firing_list, latent_out_list)):
                ...
                taste_grp = hf5.create_group(rnn_output, taste_group_name, f'Taste {taste_ind}')
        ...
        for taste_index, latent_out in enumerate(latent_out_list):
            ...
            reshaped_latents = latent_out.reshape(-1, latent_dim)  # Shape: (trial * time, latent_dim)
            ...
            trial_column = pl.Series("trial", np.repeat(np.arange(num_trials), num_time_steps))
            time_column = pl.Series("time", np.tile(np.arange(num_time_steps), num_trials))
            
            # Add these columns to DataFrame
            latent_df = latent_df.with_columns([taste_column, trial_column, time_column])
        ...
        latent_output_path = os.path.join(artifacts_dir, f"{dataset_name}_raw_latent_vectors.parquet")
