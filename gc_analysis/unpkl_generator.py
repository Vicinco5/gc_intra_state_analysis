...
def extract_valid_changepoints(pkl_path, spike_array, dataset_num, index, key):
    ...
    try:
                ...
                for idx, row in df.iterrows():
                    ...
                    if truncated_basename == truncated_dataset_num:
                        ...
                        if isinstance(extracted_row[3], np.ndarray):
                            ...
                            print(f"Skipping dataset {truncated_dataset_num} due to invalid data structure at index {idx}: {extracted_row[3]}")
                            raw_pkl = []
                            break
        ...
        if not raw_pkl:
            print(f"Skipping dataset {truncated_dataset_num} due to invalid data structures.")
            return None

        print(f"Finished processing dataset {truncated_dataset_num}. Starting to process raw_pkl.")
        ...
        for i in range(len(raw_pkl)):
            ...
            for j in range(len(new_row[3])):
                ...
                if isinstance(row, (list, np.ndarray)) and len(row) == 3:
                    ...
                    print(f"Unexpected data structure for row {j} in new_row[{i}]: {row}")
                    continue
