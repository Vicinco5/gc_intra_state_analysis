...
def calc_fr_states(spike_array, changepoints, window_length, step_size):
    def calc_fr_rr(state_spike_array, window_length, step_size):
        ...
        for bin_ini in range(num_bins):
            ...
            end_bin = min(start_bin + window_length, time_num)
            num_spikes = np.sum(state_spike_array[:, start_bin:end_bin], axis=-1)
            firing_rate[:, bin_ini] = (num_spikes / (window_length / 1000))  # Convert to Hz CHECK MATH !!! -- this may explain some of what I'm seeing
    ...
    for taste_idx in range(taste_num):
        ...
        largest_cps = np.array([max_first, max_second, max_third, max_fourth]) # pulling these out of all trials 
        for trial_idx in range(trial_num):
            trial_spike_array = spike_array[taste_idx, trial_idx, :, :]
            ...
            for cp_idx, changepoint in enumerate(trial_changepoints):
                end_time = changepoint if cp_idx < len(trial_changepoints) - 1 else (changepoint + 0)
                if end_time <= start_time:
                    print(f"No data to process for segment starting at {start_time} and ending at {end_time}. Skipping...")
                    continue
                war_end = largest_cps[cp_idx] # warped end_time rather than regular end_time
                ...
                if state_spike_array.shape[1] < 2: 
                    print(f"Not enough data points for interpolation. Skipping segment {cp_idx}...")
                    continue # forgot to add this
                # issue here with the construction of war_state_spike array-- getting odd lenghs 
                original_len = np.linspace(start_time, end_time, state_spike_array.shape[1]) # linearly spaced vectors for warping 
                ...
                war_state_spike_array = np.zeros((state_spike_array.shape[0], len(war_len)))
                
                for neuron_idx in range(state_spike_array.shape[0]): 
                     f = interp1d(original_len, state_spike_array[neuron_idx, :], kind = 'nearest', fill_value='extrapolate')
                     war_state_spike_array[neuron_idx, :] = f(war_len)
                war_firing_rate = calc_fr_rr(war_state_spike_array, window_length, step_size)
                ...
                padded_war_fr = np.pad(war_firing_rate, ((0,0), (0, max_bins - num_war_bins)), mode = 'constant', constant_values = np.nan)
                state_firing_rates_warped.loc[taste_idx, trial_idx, cp_idx, :, :] = padded_war_fr
                state_spike_arrays_warped.loc[taste_idx, trial_idx, cp_idx, :, :war_end-start_time-1] = war_state_spike_array
    ...
    return state_firing_rates, state_spike_arrays, state_firing_rates_warped, state_spike_arrays_warped
