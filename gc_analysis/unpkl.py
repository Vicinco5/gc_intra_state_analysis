...
def unpickle_changepoints(pkl_path, save_path):
    ...
    try:
        ...
        save_file_names = [truncate_name(os.path.splitext(os.path.basename(f))[0]) for f in os.listdir(save_path)]
        extracted_pkl = []
            ...
            try:
                ...
                for index, row in df.iterrows():
                    ...
                    if truncated_basename in save_file_names:
                        if truncated_basename in processed_base_names: # Skip rows that have already been processed
                            continue
                        # Append the matching row to the extracted_pkl list
                        extracted_row = row.values
                        extracted_pkl.append(extracted_row)
                        processed_base_names.add(truncated_basename)  # Mark the base name as processed
