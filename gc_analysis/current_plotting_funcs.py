...
def plt_inter_cpfr_avg_nrn_line(all_interpolated_states, modified_tastes, dataset_num):
    ...
    fig, axes = plt.subplots(num_tastes, num_changepoints, figsize=(15, 5 * num_tastes), sharex=True, sharey=True)
    fig.suptitle(f'Interpolated Firing Rates Averaged Across Neurons, Dataset: {dataset_num}', fontsize=16, y=0.995)
    ...
    for taste_idx in range(num_tastes):
        for cp_idx in range(num_changepoints):
            ...
            for trial_array in all_interpolated_states[taste_idx][cp_idx]:
                ...
                if valid_data.size > 0: # if there aren't only nan values in the original data
                ...
                # Calculate and plot the average firing rate across neurons for each trial
                    ...
                    if first_trial: 
                        ax.plot(x_ax, avg_firing_rate_across_neurons, color=colors[taste_idx], alpha=0.2, label='Trial')
                        ax.legend()
                        first_trial = False 
                    else: 
                        ax.plot(x_ax, avg_firing_rate_across_neurons, color=colors[taste_idx], alpha=0.2)
            ...
            if all_valid_trials:
                ...
                align_trials = np.array([np.pad(trial, (0, max_len - trial.shape[0]), constant_values = np.nan) for trial in all_valid_trials])
                ...
                x_ax = np.linspace(0,1,num=num_time_bins)
                ax.plot(x_ax, avg_fr_nrns, color=colors[taste_idx], linewidth=2, label='Average')
                ax.legend()
    plt.tight_layout()
    plt.show()
...
def plt_inter_cpfr_avg_tr_line(all_interpolated_states, modified_tastes, dataset_num):
    ...
    taste_labels = modified_tastes
    fig, axes = plt.subplots(num_tastes, num_changepoints, figsize=(15, 5 * num_tastes), sharex=True, sharey=True)
    fig.suptitle(f'Interpolated Firing Rates Averaged Across Trials, Dataset: {dataset_num}', fontsize=16, y=0.995)
    # Ensure axes is 2D 
    if num_tastes == 1:
        axes = np.expand_dims(axes, 0)
    for taste_idx in range(num_tastes):
        for cp_idx in range(num_changepoints):
            ...
            all_neuron_data = []
            first_trial = True
            for neuron_idx in range(all_interpolated_states[taste_idx].shape[2]):  # Loop through neurons
                neuron_trials = [] # init neuron trials 
                for trial_idx in range(all_interpolated_states[taste_idx].shape[1]):  # Loop through trials
                    trial_array = all_interpolated_states[taste_idx][cp_idx][trial_idx][neuron_idx].values
                    valid_data = trial_array[~np.isnan(trial_array)]
                    if valid_data.size > 0:  # if there are only nan values in the original data
                ...
                if neuron_trials:
                    ...
                    x_ax = np.linspace(0, 1, num=num_time_bins)
                    if first_trial: 
                        ax.plot(x_ax, avg_fr_neuron, color=colors[taste_idx], alpha=0.2, label = 'Neuron')
                        ax.legend()
                        first_trial = False
                    else: 
            ...
            if all_neuron_data:
                ...
                x_ax = np.linspace(0, 1, num=num_time_bins)
                ax.plot(x_ax, avg_fr_across_neurons, color=colors[taste_idx], linewidth=2, label='Neuron Avg')
                ax.legend()
    plt.tight_layout()
    plt.show()
...
def plt_single_neuron_firing_warped_line(all_interpolated_states, dataset_num, modified_tastes, output_dir_warped):
    num_tastes = len(all_interpolated_states)
    ...
    num_neurons = all_interpolated_states[0][0].shape[1]  # Assuming consistent neuron count across tastes and changepoints
    num_trials = all_interpolated_states[0][0].shape[0]  # Assuming consistent trial count across tastes and changepoints
    colormap = cm.get_cmap('rainbow', num_trials)  # Generate a colormap with `num_trials` colors

    # Loop through each neuron
    for neuron_idx in range(num_neurons):
        fig, axes = plt.subplots(num_tastes, num_changepoints, figsize=(15, 5 * num_tastes), sharex=True, sharey=True)
        fig.suptitle(f'Single Neuron Firing Rates, Neuron: {neuron_idx + 1}, Dataset: {dataset_num}', fontsize=16, y=0.995)
...
def plt_single_neuron_firing_unwarped_line(state_firing_rates_all_trials, dataset_num, output_dir_unwarped, modified_tastes, epoch_labels, step_size):
    ...
    os.makedirs(dataset_dir, exist_ok=True)
    num_neurons = state_firing_rates_all_trials[0][0].shape[1]  # Assuming consistent neuron count across tastes and changepoints
    num_trials = state_firing_rates_all_trials[0].shape[0]  # Assuming consistent trial count across tastes and changepoints
    colormap = cm.get_cmap('rainbow', num_trials)  # Generate a colormap with `num_trials` colors
    # Loop through each neuron
    for neuron_idx in range(num_neurons):
        fig, axes = plt.subplots(num_tastes, num_changepoints, figsize=(15, 5 * num_tastes), sharex=False, sharey=False)
        fig.suptitle(f'Unwarped Single Neuron Firing Rates, Neuron: {neuron_idx + 1}, Dataset: {dataset_num}', fontsize=16, y=0.995)
        # Ensure axes is 2D
        if num_tastes == 1:
            axes = np.expand_dims(axes, 0)
        for taste_idx in range(num_tastes):
            ...
            for cp_idx in range(num_changepoints):
                ...
                for trial_idx in range(num_trials):  # Loop through trials
                    ...
                    if valid_data.size > 0:  # Only plot if there are valid (non-NaN) values
                        trial_color = colormap(trial_idx)  # Get a unique color for each trial
                        num_time_bins = valid_data.shape[0] # appears that we're getting repeated lengths? 
                        x_ax = np.arange(num_time_bins) * step_size  # Convert to milliseconds
                        ax.plot(x_ax, valid_data, color=trial_color, alpha=0.5)
                        trial_lengths.append(num_time_bins) # record how long a trial is
                if trial_lengths: 
                ...
                avg_duration = avg_duration # I don't actually think start/end times are needed
...
def plt_single_neuron_firing_unwarped_heatmap(state_firing_rates_all_trials, dataset_num, output_dir_unwarped, window_length, step_size, modified_tastes, epoch_labels):
    num_tastes = len(state_firing_rates_all_trials)
    num_changepoints = 3  # Number of changepoints; hardcoded but can be made flexible
    taste_labels = modified_tastes
    # Create the output directory if it doesn't exist
        ...
        for taste_idx in range(num_tastes):
            ...
            for cp_idx in range(num_changepoints):
                ...
                if trial_data:
                    ...
                    # Plot heatmap
                    im = ax.imshow(heatmap_data, aspect='auto', cmap='viridis', origin='lower', extent=[0, max_length * step_size, 0, num_trials])
                    ...
                    for i, percentile in enumerate(percentiles):
                        ...
                        ax.text(line_position, num_trials - 1, f'{labels[i]} percentile', rotation=90, verticalalignment='top', color='c', alpha=1, fontsize = 12)
...
def plt_changepoints_scat(changepoints, modified_tastes, dataset_num):
    ...
    fig.suptitle(f'Changepoints for each Taste, Dataset: {dataset_num}', fontsize=9, y=0.993)
    ...
    for taste_idx in range(num_tastes):
        ...
        for trial_idx, changepoint in enumerate(sorted_changepoints):
            ...
            for cp_idx, cp_time in enumerate(changepoint):
                ...
                ax.scatter(adjusted_cp_time, trial_idx, color=changepoint_colors[cp_idx], s=10, label=f'CP{cp_idx+1}' if trial_idx == 0 else "") 
...
def plt_changepoint_hist(changepoints, modified_tastes, dataset_num):
    ...
    fig.suptitle(f'Changepoint Durations for each Taste, Dataset: {dataset_num}', fontsize=9, y=0.993)
    changepoint_colors = ['blue', 'red', 'green']  # Colors for each changepoint
    ...
    for taste_idx in range(num_tastes):
        ...
        for cp_idx, durations in enumerate(cp_durations):
            ax.hist(durations, bins=30, color=changepoint_colors[cp_idx], alpha=0.5, label=f'CP{cp_idx+1}')
