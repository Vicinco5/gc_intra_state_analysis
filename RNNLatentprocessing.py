...
class RNNLatentProcessor:
    def __init__(self, parquet_dir, npz_path, info_path, pkl_path, taste_replacements,
            ...
            for taste in unique_tastes:
                for trial in unique_trials:
                    ...
                    if trial_data.height > 0:
                        trial_lookup[key] = trial_data.select(latent_columns).to_numpy().T  # (latent_dim, time)
...
# original, non-refactored version: 
    ...
    def extract_changepoints(self):
        for data in extract_from_npz(self.npz_path):
            if isinstance(data, tuple): 
                ...
                extracted_pkl = extract_valid_changepoints(self.pkl_path, spike_array, dataset_num, index, key)

                if extracted_pkl is not None:
                    try:
                        changepoints = extracted_pkl[:,3]  # Keep full array, not just one column
                        self.changepoints_dict[dataset_name] = changepoints
                    except IndexError:
                        print(f"Index error with dataset {dataset_name}: shape {extracted_pkl.shape}")
                else:
    ...
    def split_epochs_by_changepoints(self):
        ...
        for dataset_name, data_array in self.processed_data.items():
            ...
            all_epochs = []
            time_in_ms = [(t * self.bin_size_ms) + self.start_time_ms for t in range(num_time_steps)]
            ...
            for taste_idx in range(num_tastes):
                if len(changepoints[taste_idx]) < num_trials:
                    print(f"Mismatch in trial dimension for dataset {core_dataset_name}, taste {taste_idx}")
                    continue
                ...
                for trial_idx in range(num_trials):
                    ...
                    for cp_idx, changepoint_ms in enumerate(trial_changepoints):
                        ...
                        epoch_data = data_array[taste_idx, start_idx:end_idx, :, trial_idx] # (time, latent)

                        epoch_df = pl.DataFrame(epoch_data, schema=[f'latent_dim_{i}' for i in range(latent_dim)])
    ...
    def run_robust_pca_analysis(
        ...
        if derivative_source == "threshold":
            ...
            raise ValueError("Invalid derivative_source. Must be 'threshold' or 'full'.")
        ...
        for analysis_name, analysis_data in analysis_dicts.items():
            ...
            for dataset_name, df in analysis_data.items():
                if isinstance(df, pl.DataFrame):
                    ...
                    print(f"[WARNING] Skipping {dataset_name} for {analysis_name} — not a Polars DataFrame.")
    ...
    def full_pipeline(
        ...
        if return_derivatives:
            return self.epoch_dataframes_dict, self.robust_pca_95, self.robust_pca_full, self.first_derivs, self.second_derivs
        else:
