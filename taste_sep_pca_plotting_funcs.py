        ...
        for taste in unique_tastes:
            for changepoint in unique_changepoints:
                try:
                    ...
                    split_pca_result = unconcatenate_pca_results_taste(pca_results_df, dataset, taste, changepoint)
                    
                    # Average the first 3 principal components for each trial-- in this case just one
                    averaged_pca_results = [np.mean(trial_pca[:, :1], axis=1) for trial_pca in split_pca_result]
                    ...
                    ax.set_title(f'Averaged PCA Trajectories for Dataset: {dataset}, Taste: {taste}, Changepoint: {changepoint}')
                   # ax.legend()
                    
                    plt.show()
                except Exception as e:
                    print(f"Could not plot PCA trajectories for dataset {dataset}, taste {taste}, changepoint {changepoint}: {e}")
...
def plot_averaged_pca_trajectories_thresholded(pca_results_df, pca_output_dir, modified_tastes, epoch_labels, step_size):
    def unconcatenate_pca_results_taste(pca_results_df, dataset, taste, changepoint):
        # Filter the DataFrame for the specific dataset, taste, and changepoint
        filtered_df = pca_results_df.filter(
            (pl.col('dataset') == dataset) &
            ...
            for cp_idx, changepoint in enumerate(unique_changepoints):
                try:
                    ...
                    if filtered_df.shape[0] == 0:
                        print(f"No data for dataset {dataset}, taste {taste}, changepoint {changepoint}")
                        continue
                    ...
                    averaged_pca_results = [np.mean(trial_pca[:, :num_pc_avg], axis=1) for trial_pca in split_pca_result]
                    
                    trial_lengths = []
                    trial_data = []
                    
                    for trial_idx, averaged_pca in enumerate(averaged_pca_results):
                        trial_color = cm.get_cmap('rainbow', len(averaged_pca_results))(trial_idx)
                    ...
                    ax.plot(mean_time_bins, mean_data, color='black', linewidth=1, label='Trial Average')
                    ax.legend()
...
def plot_averaged_pca_trajectories_thresholded_SEM(pca_results_df, pca_output_dir, modified_tastes, epoch_labels, step_size):
    def unconcatenate_pca_results_taste(pca_results_df, dataset, taste, changepoint):
        # Filter the DataFrame for the specific dataset, taste, and changepoint
        filtered_df = pca_results_df.filter(
            (pl.col('dataset') == dataset) &
...
def plot_averaged_pca_heatmaps_all_taste(pca_results_df, pca_output_dir, modified_tastes, epoch_labels, step_size):
    def unconcatenate_pca_results_taste(pca_results_df, dataset, taste, changepoint):
        # Filter the DataFrame for the specific dataset, taste, and changepoint
        filtered_df = pca_results_df.filter(
            (pl.col('dataset') == dataset) &
            ...
            for cp_idx, changepoint in enumerate(unique_changepoints):
                try:
                   ...
                   # trial_lengths = [step_size * x for x in trial_length]
                    ...
                    sorted_averaged_pca_results = [averaged_pca_results[i] for i in sorted_indices]
                    
                    # Pad the trials to the same length
                    max_length = max(len(trial) for trial in sorted_averaged_pca_results)
                    padded_pca_results = [np.pad(trial, (0, max_length - len(trial)), 'constant', constant_values=np.nan) for trial in sorted_averaged_pca_results]
                    ...
                    for i, percentile in enumerate(duration_percentiles): 
                        ...
                        ax.text(percentile, ax.get_ylim()[1] * 0.9, f'{labels[i]} percentile', rotation=90, verticalalignment='top', color='grey', alpha=1)
                    
                    ax.set_title(f'Taste: {taste_labels[taste_idx]}, Epoch: {epoch_labels[cp_idx]}\nAverage Epoch Duration: {avg_duration:.2f} ms')
                
                except Exception as e:
                    print(f"Could not plot PCA heatmaps for dataset {dataset}, taste {taste}, changepoint {changepoint}: {e}")
...
def plot_pca_scatter(pca_results_df, pca_output_dir, modified_tastes, epoch_labels, step_size):
    def unconcatenate_pca_results_taste(pca_results_df, dataset, taste, changepoint):
        # Filter the DataFrame for the specific dataset, taste, and changepoint
        filtered_df = pca_results_df.filter(
            (pl.col('dataset') == dataset) &
            ...
            for cp_idx, changepoint in enumerate(unique_changepoints):
                try:
                    ...
                    for trial_idx, trial_pca in enumerate(split_pca_result):
                        ...
                        ax.scatter(trial_pca[:, 0], trial_pca[:, 1], color=trial_color, alpha=0.5, label=f'Trial {trial_idx + 1}')
                    
                    if taste_idx == num_tastes - 1 and cp_idx == num_changepoints - 1:
                        ax.legend(loc='upper right', bbox_to_anchor=(1.5, 1))

                except Exception as e:
                    print(f"Could not plot PCA scatter plots for dataset {dataset}, taste {taste}, changepoint {changepoint}: {e}")
...
def plot_pca_scatter_3d(pca_results_df, pca_output_dir, modified_tastes, epoch_labels, step_size):
    def unconcatenate_pca_results_taste(pca_results_df, dataset, taste, changepoint):
        # Filter the DataFrame for the specific dataset, taste, and changepoint
        filtered_df = pca_results_df.filter(
            (pl.col('dataset') == dataset) &
    ...
    for dataset in unique_datasets:
        ...
        axes = [[fig.add_subplot(num_tastes, num_changepoints, i * num_changepoints + j + 1, projection='3d') for j in range(num_changepoints)] for i in range(num_tastes)]
        ...
        for taste_idx, taste in enumerate(unique_tastes):
            for cp_idx, changepoint in enumerate(unique_changepoints):
                try:
                    ...
                    for trial_idx, trial_pca in enumerate(split_pca_result):
                        trial_color = colormap(trial_idx)
                        ax.scatter(trial_pca[:, 0], trial_pca[:, 1], trial_pca[:, 2], color=trial_color, alpha=0.5, label=f'Trial {trial_idx + 1}')
                        ax.plot(trial_pca[:, 0], trial_pca[:, 1], trial_pca[:, 2], color=trial_color, alpha=0.5)
                    
                    if taste_idx == num_tastes - 1 and cp_idx == num_changepoints - 1:
                        ax.legend(loc='upper right', bbox_to_anchor=(1.5, 1))

                except Exception as e:
                    print(f"Could not plot PCA 3D scatter plots for dataset {dataset}, taste {taste}, changepoint {changepoint}: {e}")
                ...
                for trial_pca in split_pca_result:
                    ...
                    if num_points > 1:
                        for i in range(num_points - 1):
                            distances.append(euclidean(trial_pca[i, :3], trial_pca[i + 1, :3]))
...
def plot_euclidean_distances_bar(distance_df, pca_output_dir, modified_tastes, epoch_labels, step_size):
    ...
    for dataset in unique_datasets:
        fig, axes = plt.subplots(num_tastes, num_changepoints, figsize=(15, 5 * num_tastes), sharex=False, sharey=False)
        fig.suptitle(f'Euclidean Distances (dist = 0 counts single points only), Dataset: {dataset}', fontsize=14, y=0.995)
        ...
        for taste_idx, taste in enumerate(unique_tastes):
            for cp_idx, changepoint in enumerate(unique_changepoints):
                try:
                    ...
                    euclidean_distances_list = filtered_df['euclidean_distances'].to_list()
                    
                    # Flatten the list of lists
                    euclidean_distances = [item for sublist in euclidean_distances_list for item in sublist]
                    ...
                    valid_distances = np.array([x for x in euclidean_distances if not np.isnan(x)])
                    mean_distance = np.mean(valid_distances) if valid_distances.size > 0 else np.nan
                    std_distance = np.std(valid_distances) if valid_distances.size > 0 else np.nan
                    ...
                    print(f"Could not plot Euclidean distances for dataset {dataset}, taste {taste}, changepoint {changepoint}: {e}")

        plt.tight_layout()
        bar_fig_path = os.path.join(bar_output_dir, f'{dataset}_euclidean_distances_bar.png')
...
def plot_explained_variance_combined_by_taste(pca_results_df, pca_output_dir, modified_tastes, epoch_labels):
    unique_datasets = pca_results_df['dataset'].unique().to_list()
    unique_tastes = pca_results_df['taste'].unique().to_list()
    unique_changepoints = pca_results_df['changepoint'].unique().to_list()
        ...
        for taste_idx, taste in enumerate(unique_tastes):
            for cp_idx, changepoint in enumerate(unique_changepoints):
                try:
                    ...
                    explained_variance_ratio = filtered_df['explained_variance_ratio'][0].to_numpy()

                    # Principal components
                    pcs = np.arange(len(explained_variance_ratio))

                    ax = axes[taste_idx, cp_idx]
                    ax.bar(pcs, explained_variance_ratio, color='b', alpha=0.6, label='Explained Variance Ratio')
                    ...
                    ax.plot(pcs, cumulative_explained_variance, label='Cumulative Explained Variance', color='r', marker='o')
                    ...
                    print(f"Could not plot explained variance ratio for dataset {dataset}, taste {taste}, changepoint {changepoint}: {e}")

        plt.tight_layout()
        explained_variance_fig_path = os.path.join(explained_variance_output_dir, f'{dataset}_explained_variance.png')
