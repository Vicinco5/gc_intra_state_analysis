...
spike_train[0, changepoints[0]:changepoints[1]] = np.random.binomial(1, 0.5, changepoints[1] - changepoints[0])

# State 3: Moderate firing rate
spike_train[0, changepoints[1]:] = np.random.binomial(1, 0.3, total_time - changepoints[1])
...
ax[0].plot(np.arange(firing_rate.shape[1]) * step_size + window_length/2, firing_rate[0], color='black')
...
for i, cp in enumerate(changepoints):
    ax[0].axvspan(cp, changepoints[i+1] if i+1 < len(changepoints) else total_time, color=colors[i+1], alpha=0.3)
    ax[1].axvspan(cp, changepoints[i+1] if i+1 < len(changepoints) else total_time, color=colors[i+1], alpha=0.3)

# Plot the spike train
ax[1].eventplot(np.where(spike_train[0] == 1)[0], orientation='horizontal', colors='black')
...
def calc_fr_rr(state_spike_array, window_length, step_size):
    ...
    for bin_ini in range(num_bins):
        ...
        end_bin = min(start_bin + window_length, time_num)
        num_spikes = np.sum(state_spike_array[:, start_bin:end_bin], axis=-1)
        firing_rate[:, bin_ini] = num_spikes / (window_length * 0.001)  # Convert to Hz (window in ms)
    return firing_rate
...
neurons = []
target_firing_rates = np.random.uniform(4, 7, size=4)  # Random target FR between 3 and 7 Hz
...
for nrn_idx in range(neurons_padded.shape[0]):
    ...
    padded_fr = np.full((fr.shape[0], (max_length - window_length) // step_size + 1), np.nan)
...
ax.plot(time_vector, avg_firing_rate, color='black', linewidth=2.5, label='Average firing rate')
...
for idx in range(firing_rates.shape[0]):
    ...
    if len(old_time_valid) < 2:
        ...
        interpolator = interp1d(old_time_valid, fr_valid, kind='nearest', fill_value='extrapolate')
...
ax.plot(new_time, avg_firing_rate_stretched, color='black', linewidth=2.5, label='Average firing rate (stretched)')
...
for dur, fr_target in zip(neuron_durations, base_firing_rates):
    ...
    dynamic_spike_prob = np.clip(spike_prob_per_ms * (1 + modulation), 0.0005, 0.05)  # Cap at realistic spike probs
...
for idx in range(firing_rates.shape[0]):
    line, = ax.plot(time_vector, firing_rates[idx], label=f'Neuron {idx+1}', alpha=0.7, color=colors[idx % len(colors)])
    ...
    if np.any(valid_mask):
        ...
        ax.axvline(x=last_valid_time, color=colors[idx % len(colors)], linestyle='--', alpha=0.7)
...
for idx in range(firing_rates_stretched.shape[0]):
    ax.plot(new_time, firing_rates_stretched[idx], label=f'Neuron {idx+1}', alpha=0.7, color=colors[idx % len(colors)])

ax.plot(new_time, avg_firing_rate_stretched, color='black', linewidth=2.5, label='Average firing rate (warped)')
...
for neuron_idx in range(n_neurons):
    ...
    first_half_modulation = 0.2 * np.sin(2 * np.pi * 2 * time_first) + 0.5 * np.random.randn(first_half_duration)
    second_half_modulation = 0.2 * np.sin(2 * np.pi * 3 * time_second) + 0.5 * np.random.randn(second_half_duration)

    first_half_spike_prob = np.clip(first_half_base_rate * (1 + first_half_modulation), 0.0005, 0.05)
    second_half_spike_prob = np.clip(second_half_base_rate * (1 + second_half_modulation), 0.0005, 0.05)
...
if p_value < 0.05:
    print("✅ Statistically significant difference between halves (average firing rate).")
else:
