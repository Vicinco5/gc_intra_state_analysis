                    ...
                    try:
                        ...
                        if unaligned_mode == "none":
                            ...
                            for j, row_data in enumerate(valid_trials):
                                # Timing info
                                trial_cp = standardized_changepoints_dict[dataset_name][taste_idx][j]
                                trial_start = 2000 if cp_idx == 0 else trial_cp[cp_idx - 1]
                                ...
                                if offset_bins + len(row_data) <= global_bins:
                                    all_data[j, offset_bins:offset_bins + len(row_data)] = row_data
                                else:
                                    # If it would overrun the array (rare), truncate
                                    available_bins = global_bins - offset_bins
                                    all_data[j, offset_bins:] = row_data[:available_bins]
                            ...
                            for j, row_data in enumerate(valid_trials):
                                ...
                                if unaligned_mode == 'start' or use_warped:
                                    ...
                                    all_data[j, max_len - row_len:] = row_data # bug fix I think
                                else:  # fallback
                                    all_data[j, :row_len] = row_data
...
def find_dataset_in_sig_dict(sig_nrns_dict, dataset_num_clean):
    ...
    for df_name, df in sig_nrns_dict.items():
        ...
        matching_rows = df.filter(pl.col("dataset").cast(pl.Utf8).str.contains(dataset_num_clean))
...
def prompt_plot_flags():
    ...
    if plot_unwarped:
        ...
        unaligned_mode = input("Enter alignment mode (start/end/none): ").strip().lower()
        ...
        if isinstance(data, tuple) and len(data) == 4:
            ...
            if extracted_pkl is not None:
                try:
                    ...
                    dataset_output_dir = os.path.join(base_output_dir, f"dataset_{dataset_num_clean}")
                    ...
                    if plot_unwarped:
                        plot_neuron_mean_with_sem_grid_single_dataset(
                            ...
                            standardized_changepoints_dict=standardized_changepoints_dict
                                if unaligned_mode == "none" else None
                        )
