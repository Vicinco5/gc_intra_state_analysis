...
def whitened_pca_all_nrns_all_taste(all_data_df):
    ...
    for dataset in unique_datasets:
        ...
        for changepoint in unique_changepoints:
            ...
            for neuron in unique_neurons:
                neuron_df = dataset_df.filter((pl.col('changepoint') == changepoint) & (pl.col('neuron') == neuron))
                trials_data = neuron_df['trial_data'].to_list()
                
                # Ensure trial data is 2D and store original lengths
                trials_data = [np.array(trial).reshape(1, -1) if np.array(trial).ndim == 1 else np.array(trial) for trial in trials_data]
            ...
            data_dict = {str(neuron): concatenated_data_list[i].flatten() for i, neuron in enumerate(unique_neurons)}
            ...
            if np.any(np.std(standardized_data, axis=0) > 0):
                ...
                all_pca_results.append((dataset, changepoint, pca_result, explained_variance, singular_values, trial_lengths))
            else:
                print(f"Skipping PCA for dataset {dataset}, changepoint {changepoint} due to lack of variation.")
    ...
    pca_results_df = pl.DataFrame({
        ...
        'pca_result': [result[2].tolist() for result in all_pca_results],  # Store as list
        'explained_variance_ratio': [result[3].tolist() for result in all_pca_results],
        'singular_values': [result[4].tolist() for result in all_pca_results],
        'trial_lengths': [result[5] for result in all_pca_results]  # Store trial lengths
    })
    ...
    for dataset in unique_datasets:
        for changepoint in unique_changepoints:
            try:
                ...
                result = pca_results_df.filter((pl.col('dataset') == dataset) & (pl.col('changepoint') == changepoint)).row(0)
                ...
                split_pca_result = unconcatenate_pca_results_all(pca_result, trial_lengths)
                
                # Average each PCA result array along neurons
                averaged_pca_results = [np.mean(trial_pca, axis=1) for trial_pca in split_pca_result]
                ...
                ax.set_title(f'Averaged PCA Trajectories for Dataset: {dataset}, Changepoint: {changepoint}')
                ax.legend()
                
                plt.show()
            except Exception as e:
                print(f"Could not plot PCA trajectories for dataset {dataset}, changepoint {changepoint}: {e}")
    ...
    for dataset in unique_datasets:
        for changepoint in unique_changepoints:
            try:
                ...
                # Unconcatenate PCA results
                split_pca_result = unconcatenate_pca_results_all(pca_results_df, dataset, changepoint)
    ...
    for dataset in unique_datasets:
        for changepoint in unique_changepoints:
            try:
                ...
                plt.title(f'PCA Scatter Plot for Dataset: {dataset}, Changepoint: {changepoint}')
                plt.show()
            except Exception as e:
                print(f"Could not plot PCA scatter for dataset {dataset}, changepoint {changepoint}: {e}")
    ...
    for dataset in unique_datasets:
        for changepoint in unique_changepoints:
            try:
                ...
                loadings = pca_result * singular_values / np.sqrt(pca_result.shape[0] - 1)
                
                # Create scatter plot of the first two principal components
                plt.figure(figsize=(10, 6))
                plt.scatter(pca_result[:, 0], pca_result[:, 1], alpha=0.7, label='Samples')
                
                # Add loading vectors
                for i, vector in enumerate(loadings[:2].T):
                    plt.arrow(0, 0, vector[0], vector[1], color='r', alpha=0.5)
                    plt.text(vector[0] * 1.2, vector[1] * 1.2, f"Var{i+1}", color='r', ha='center', va='center')

                plt.xlabel('Principal Component 1')
                plt.ylabel('Principal Component 2')
                plt.title(f'PCA Biplot for Dataset: {dataset}, Changepoint: {changepoint}')
                plt.legend()
                plt.show()
            except Exception as e:
                print(f"Could not plot PCA biplot for dataset {dataset}, changepoint {changepoint}: {e}")
    ...
    for dataset in unique_datasets:
        for changepoint in unique_changepoints:
            try:
                ...
                ax.set_title(f'PCA 3D Scatter Plot for Dataset: {dataset}, Changepoint: {changepoint}')
                plt.show()
            except Exception as e:
                print(f"Could not plot PCA 3D scatter for dataset {dataset}, changepoint {changepoint}: {e}")
    ...
    for dataset in unique_datasets:
        for changepoint in unique_changepoints:
            try:
                ...
                ax2 = ax1.twinx()
                ax2.plot(pcs, cumulative_explained_variance, label='Cumulative Explained Variance', color='r', marker='o')
                ax2.set_ylabel('Cumulative Explained Variance Ratio', color='r')
                ax2.set_ylim(0, 1.1)
                ax2.legend(loc='upper left')
                
                plt.title(f'Explained Variance Ratio per PC for Dataset: {dataset}, Changepoint: {changepoint}')
                plt.show()
            except Exception as e:
                print(f"Could not plot explained variance ratio for dataset {dataset}, changepoint {changepoint}: {e}")
    ...
    for dataset in unique_datasets:
        for changepoint in unique_changepoints:
            try:
                ...
                # Bar plot for explained variance ratio
                ax1.bar(pcs, explained_variance_ratio, color='b', alpha=0.6, label='Explained Variance Ratio')
                ax1.set_xlabel('Principal Components')
                ...
                # Line plot for cumulative explained variance ratio
                cumulative_explained_variance = np.cumsum(explained_variance_ratio)
                ax1.plot(pcs, cumulative_explained_variance, label='Cumulative Explained Variance', color='r', marker='o')
