...
class RNNWarpingIntraAroundCP:
    ...
    def get_data_columns(df: pl.DataFrame):
        return [c for c in df.columns if c.startswith('PC_') or c.startswith('latent_dim_')]
    ...
    def extract_changepoints(self, core_name: str):
        ...
        if cp_list is None:
            ...
            return None
        return [np.clip(np.array(arr), self.start_time, self.end_time) for arr in cp_list]
    ...
    def align_trials(self, df: pl.DataFrame):
        ...
        for taste, trial in df.select(['taste','trial']).unique().rows():
            ...
            aligned[(taste,trial)] = grp.with_columns((pl.col('time')-self.start_time).alias('time'))
        return aligned

    def _plot_non_windowed(self, dataset_name: str, df: pl.DataFrame, dims: list, cp_list: list):
        ...
        for taste_idx, taste in enumerate(tastes):
            ...
            for row, dim in enumerate(dims):
                for eid in range(num_epochs):
                    ...
                    for trial in trials:
                        ...
                        seg=trial_df.filter((pl.col('time')>=start)&(pl.col('time')<=end))
                        t=seg['time'].to_numpy()-start; v=seg[dim].to_numpy()
                        if t.size>0:
                            if self.mode=='unwarped':
                                ...
                                wv=f(tgt); ta=np.linspace(0,self.warp_length,self.warp_length)
                    ...
                    if self.plot_average and epoch_vals:
                            ...
                            ax.plot(grid, np.nanmean(mat, axis=0), linewidth=2, alpha=1, color='gray')
                    ax.set_title(f"Epoch {eid}\n{dim}")
                    ax.set_xlabel('Time (ms)'); ax.set_ylabel('PC magnitude')
            fig.suptitle(f"Mode: {self.mode}, Taste: {taste}, Dataset: {dataset_name} (4 epochs)")
            ann=("Epoch 0: start→1st CP; Epoch 1: 1st→2nd; "
            ...
            fig.tight_layout(); fig.subplots_adjust(top = 0.95, bottom=0.06)
            fig.text(0.5,0.02,ann,ha='center',va='top',fontsize=16)
            od=os.path.join(self.output_dir,dataset_name,f"taste_{taste}"); os.makedirs(od,exist_ok=True)
            fig.savefig(os.path.join(od,f"{self.mode}_taste_{taste}_{dataset_name}.png")); plt.close(fig)

    def _plot_windowed(self, dataset_name: str, df: pl.DataFrame, dims: list, cp_list: list):
            ...
            for col in range(cp_cols):
                for row, pc in enumerate(pc_columns):
                    ...
                    for trial in trials:
                        ...
                        v = seg[pc].to_numpy()
                        if t.size>0:
                            ax.plot(t, v, linewidth=1) # change this to a rainbow-esque thing later on 
                            trial_vals.append((t, v))
                    if self.plot_average and trial_vals:
                        grid = trial_vals[0][0]
                        arr = np.vstack([
                            np.interp(grid, t, v) for t, v in trial_vals
                        ])
                        ax.plot(grid, np.nanmean(arr, axis=0), color='gray', alpha = 1, linewidth= 2)
