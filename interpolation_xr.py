    ...
    for taste_idx in range(state_firing_rates.sizes['taste']):
        ...
        for state_idx in range(taste_data.sizes['segment']):
            ...
            for trial_idx in range(segment_data.sizes['trial']):
                ...
                interpolated_array = np.full((trial_data.sizes['neuron'], median_length), np.nan)  # Prepare container with NaNs
                ...
                for nrn_idx in range(trial_data.sizes['neuron']):
                    ...
                    if valid_mask.sum() > 1:  # Need at least 2 points to interpolate
                        ...
                        interpolated_array[nrn_idx] = interp_func(np.linspace(0, len(neuron_data) - 1, median_length))
            ...
            interpolated_xr = xr.DataArray(
                ...
                coords={'trial': range(segment_data.sizes['trial']), 'neuron': range(trial_data.sizes['neuron']), 'time_bin': range(median_length)}
            )
        ...
        for seg_idx in range(segment_num):
            for nrn_idx in range(nrn_num):
                ...
                for trial_idx in range(trial_num):
                    fr_vector = state_firing_rates[taste_idx, trial_idx, seg_idx, nrn_idx, :].values
                ...
                for trial_idx in range(trial_num):
                    ...
                    if not np.isnan(fr_vector).all():  # Skip if all values are nan
                        ...
                        interpolator = interp1d(time_original, fr_vector[~np.isnan(fr_vector)], kind='linear')
                        ...
                        if len(interpolated_vector) > max_bins:
                            ...
                            interpolated_vector = np.pad(interpolated_vector, (0, max_bins - len(interpolated_vector)), mode='constant', constant_values=np.nan)
                        
                        # Store the interpolated vector in the new xarray
                        interpolated_firing_rates.loc[taste_idx, trial_idx, seg_idx, nrn_idx, :] = interpolated_vector
